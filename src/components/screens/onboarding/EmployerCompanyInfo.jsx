...
const EmployerCompanyInfo = () => {
  ...
  const fetchWorkTools = async () => {
    ...
    if (response?.status === "success") {
      setWorkTools(
        response?.data.map((tool) => ({ value: tool.name, label: tool.name }))
      );
    } else {
      console.log(response);
    }
  ...
  const profileUpdate = async () => {
    try {
      ...
      if (update?.status === "success") {
        ...
        navigate(
          hireId ? `/select-service?hireId=${hireId}` : "/select-service"
        );
      } else {
        if (update.error) {
  ...
  const validateProfileForm = async () => {
    const validate = await runValidation([
      {
        input: {
          value: organization?.employee_size,
          field: "company_size",
          type: "text",
        },
        rules: { required: true },
      ...
      {
        input: {
          value: organization?.years_in_operation.toString(),
      ...
      {
        input: {
          value: organization?.technology_tools,
  ...
  useEffect(() => {
    setSelectedOptions(
      organization?.technology_tools?.map((tool) => ({
        value: tool,
        label: tool,
      }))
    );
        ...
        <div className="flex items-start justify-center min-h-[680px] mobilelandscape:flex-nowrap flex-wrap">
          <div className="xl:w-6/12 lg:w-5/12 mobilelandscape:w-5/12 mobilelandscape:inline-flex hidden ">
            <img
              ...
              alt=""
              className=" min-h-[680px] w-full tabletmd:object-none object-cover"
            />
          </div>
          <div className="xl:w-6/12 lg:w-7/12 mobilelandscape:w-7/12 mobilelg:w-11/12 w-full bg-white xl:px-8 px-4 pt-10  ">
            <div className="mobilesm:p-6 p-0    w-full">
              ...
              <p className="text-base text-text-center font-normal text-[#3B4752] ">
              ...
              <form className="space-y-3 md:space-y-5 mt-4" action="#">
                ...
                <div className="flex items-start mobilelg:flex-nowrap flex-wrap space-x-10 w-full space-y-4">
                  ...
                  <div className="mobilelg:w-6/12 w-full">
                    ...
                    <select
                      ...
                      className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 outline-none bg-ftvwine-25 focus:bg-ftvgrey-25 border-ftvgrey-200  placeholder-gray-400 text-ftvblack-300 focus:ring-ftvwine-300 focus:border-ftvwine-200 "
                      onChange={(e) =>
                        updateOrganization("employee_size", e.target.value)
                      }>
                ...
                <div className="flex items-start mobilelg:flex-nowrap flex-wrap space-x-10 w-full space-y-4">
                  <div className="mobilelg:w-12/12 w-full">
                    <label
                      htmlFor="technology_took"
                      className="block mb-2 text-sm font-medium text-gray-900 text-ftvblack-400">
                      What tools, software, or platforms do you use?{" "}
